...
$(document).ready(function () {
  const observer = new MutationObserver(checkSnipcartWrapper);
  const radioInPost = $("input[type=radio][value=inpost]");
  const radioRegular = $("input[type=radio][value=regularna-wysyka]");
  const geowidget = $('<div class="custom-geowidget"></div>');

  function checkSnipcartWrapper(mutationsList, observer) {
    if ($("#snipcart .snipcart-layout--large").length) {
      handleButtons(observer);
    }
  }

  function handleButtons(observer) {
    const radioButtons = $("input[type=radio]");

    if (radioButtons.length) {
      checkButtonStateAndChange(observer);
    }

    radioInPost.on("change", buttonClickHandler("disabled", 1200));
    radioRegular.on("change", buttonClickHandler("disabled", 500));
  }

  function checkButtonStateAndChange(observer) {
    checkRadioButtonState();
    radioButtons.on("change", function () {
      radioInPost.prop("checked") ? showAndAppendGeowidget(observer) : hideGeowidget();
    });
  }

  function checkRadioButtonState() {
    if (radioInPost.prop("checked")) {
      radioInPost.prop("checked") && showAndAppendGeowidget();
    }
  }

  function showAndAppendGeowidget(observer) {
    showGeowidget();
    appendGeoWidget(observer);
  }

  function buttonClickHandler(attr, timeout) {
    return function () {
      setTimeout(() => getContinueToPaymentButton().attr(attr, attr), timeout);
    };
  }
  ...
  function showGeowidget() {
    geowidget.css("display", "block");
  }

  function hideGeowidget() {
    geowidget.css("display", "none");
  }

  function appendGeoWidget(observer) {
    if (geowidgetAppended) return;
    var userLang = (navigator.language || navigator.userLanguage).substr(0, 2);
    if (userLang !== "pl") {
      userLang = "en";
