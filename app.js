...
$(document).ready(function () {
  ...
  function appendGeoWidget() {
    ...
    geowidget.addEventListener("handlePointSelection", async (event) => {
      ...
      console.log(globalBillingDetails);
      try {
        await Snipcart.api.cart.update({
          email: "john.doe@snipcart.com",
          shipToBillingAddress: false,
          billingAddress: {
            name: globalBillingDetails.fullName,
            firstName: globalBillingDetails.fullName,
...
document.addEventListener("snipcart.ready", function () {
  Snipcart.events.on("cart.confirmed", function (order) {
    ...
    $.ajax({
      ...
      data: JSON.stringify({
        receiver: {
          first_name: globalBillingDetails.fullName,
          last_name: globalBillingDetails.fullName,
